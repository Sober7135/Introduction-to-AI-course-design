...
def hidden_layer_output():
    ...
    for i in range(hidden_outputs.shape[1]):  # hidden_outputs.shape[1] 是神经元的数量
        plt.subplot(2, 2, i + 1)
        plt.scatter(
            coordinates[:, 0], coordinates[:, 1], c=hidden_outputs[:, i], cmap="viridis"
        )
        plt.colorbar()
...
def weights_loss():
    weights_fc1, weights_fc2, losses = train_with_feedback(
        epochs, coordinates_train, labels_train
    )
    weights_fc1, weights_fc2, losses = (
        np.array(weights_fc1),
        np.array(weights_fc2),
        np.array(losses),
    )
    ...
    for neuron_index in range(weights_fc1.shape[1]):
        for weight_index in range(weights_fc1.shape[2]):
            plt.plot(
                weights_fc1[:, neuron_index, weight_index],
                label=f"Neuron {neuron_index} Weight {weight_index}",
            )
    ...
    for neuron_index in range(weights_fc2.shape[1]):
        for weight_index in range(weights_fc2.shape[2]):
            plt.plot(
                weights_fc2[:, neuron_index, weight_index],
                label=f"Neuron {neuron_index} Weight {weight_index}",
            )
